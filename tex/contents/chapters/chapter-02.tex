...
Backstage is an open-source framework for building IDPs, originally developed
by Spotify and now hosted by the Cloud Native Computing Foundation (CNCF). It
has emerged as the de facto standard for building developer portals due to its
extensibility and "catalog-first" philosophy.

Backstage operates on three core pillars:
\begin{enumerate}
	\item \textbf{Software Catalog:} A centralized system that tracks ownership and metadata for all software components (services, libraries, websites, pipelines).
	\item \textbf{Software Templates:} Scaffolding tools that allow developers to spin up new projects with best practices automatically configured.
	\item \textbf{TechDocs:} A documentation-as-code solution that renders Markdown files living with the code into a searchable documentation site.
\end{enumerate}
...
directly into the developer's daily workflow. It is within this plugin
ecosystem that the proposed solution, HypoStage, is situated, leveraging the
existing adoption of IDPs to introduce architectural hypothesis management
without forcing developers to switch contexts.
